...
tournament_matches = np.array(pd.read_csv("bracket_matchups.csv", header = None))
tourney_act_results = np.array(pd.read_csv("actual_results.csv", header = None))

tourney_round = 0
tourney_predict_results = [[]]

output_file = open("bracket_predicts.txt", "w")
...
for i in range(int(len(match_data) / 2)):  # training and testing data (match dataset) is split in half
...
for i in range(0, len(team_data)):
    if team_data[i][0] in match_teams: # else: this team wasn't in the match data, so don't do anything
        ...
        for j in range(1, len(team_weights) + 1):
            ...
            # MIGHT HAVE A PROBLEM IF ANY team_feature_means OR player_feature_means VALUES ARE NEGATIVE
...
for i in range(0, len(player_data)):
    if player_data[i][1] in match_teams: # else: this player's team wasn't in the match data, so don't do anything
        ...
        #else:
            #print(str(player_data[i][1]) + ' not found in team dataset')
    #else:
        #print(str(player_data[i][1]) + ' not found in match dataset')
...
num_total = 0
missing = 0
for i in range(int(len(match_data) / 2), len(match_data)):  # training and testing data (match dataset) is split in half
    team1_val = 0
    team2_val = 0
    ind1 = -1
    ind2 = -1
    ...
    if (ind1 >= 0) and (ind2 >= 0):
        ...
        if ind1 == -1:
            #print(str(match_data[i][1]))
            missing += 1
        if ind2 == -1:
            #print(str(match_data[i][3]))
            missing += 1
    ...
    if (team1_val > team2_val) and (match_data[i][2] > match_data[i][4]):
	    ...
	    num_correct += 1 
print("Accuracy on Regular Season Games: %.6f" % (num_correct / num_total))

# Using algorithm for bracket prediction
for i in range(len(tournament_matches)):  
    ...
    ind2 = -1
    for k in range(0, len(team_data)):
        if team_data[k][0] == tournament_matches[i][1]:
            ind1 = k
            if (ind1 >= 0) and (ind2 >= 0):
                k = len(team_data)
        elif team_data[k][0] == tournament_matches[i][3]:
            ind2 = k
            if (ind1 >= 0) and (ind2 >= 0):
                k = len(team_data)
    if (ind1 >= 0) and (ind2 >= 0):
    ...
    if tournament_matches[i][1] in teams:
        team_ind = teams.index(tournament_matches[i][1])
        player_ind = -1
        for k in range(len(player_data)):
            if player_data[k][1] == tournament_matches[i][1]:
                player_ind = k
        if player_ind >= 0 and team_ind >= 0:
            for j in range(2, (len(player_weights) - 18) + 2):
                team1_val += (player_data[player_ind][j] * player_weights[j - 2]) / len(players_in_teams[team_ind])
    
    if tournament_matches[i][3] in teams:
        team_ind = teams.index(tournament_matches[i][3])
        player_ind = -1
        for k in range(len(player_data)):
            if player_data[k][1] == tournament_matches[i][1]:
                player_ind = k
    ...
    if (i == 0):
        output_file.write("===First Round===\n")
    elif (i == 32):
        output_file.write("===Second Round===\n")
        tourney_predict_results.append([])
        tourney_round += 1
    elif (i == 48):
        output_file.write("===Third Round===\n")
        tourney_predict_results.append([])
        tourney_round += 1
    elif (i == 56):
        output_file.write("===Fourth Round===\n")
        tourney_predict_results.append([])
        tourney_round += 1
    elif (i == 60):
        output_file.write("===Fifth Round===\n")
        tourney_predict_results.append([])
        tourney_round += 1
    elif (i == 62):
    ...
    team1_prob = (team1_val ** 2) / ((team1_val ** 2) + (team2_val ** 2) )
    team2_prob = (team2_val ** 2) / ((team1_val ** 2) + (team2_val ** 2) )
	
    if team1_prob > team2_prob:
        tourney_predict_results[tourney_round].append(tournament_matches[i][1])
    elif team2_prob > team1_prob:
	    tourney_predict_results[tourney_round].append(tournament_matches[i][3])
	
    output_file.write("%-21s %5.4f, %-21s %5.4f\n" % (tournament_matches[i][1]+",",team1_prob,tournament_matches[i][3]+",",team2_prob) ) 

# calculate accuracy of tournament predictions	
total_num_tourney_predictions = 32 + 16 + 8 + 4 + 2 + 1
num_tourn_pre_correct = 0

for i in range(len(tourney_predict_results)):
    for j in range(len(tourney_predict_results[i])):
	    if tourney_predict_results[i][j] in tourney_act_results[i]:
		    num_tourn_pre_correct += 1

tournament_accuracy = num_tourn_pre_correct / total_num_tourney_predictions

print("Accuracy for March Madness Tournament: %f" % (tournament_accuracy))
print("Tournament prediction probabilites stored in bracket_predicts.txt")
